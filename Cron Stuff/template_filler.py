...
def past_tensify(txt, nlp):
    ...
    all_words = [t.text if t.text in ['.',',',':',';','?','!', ')'] else " "+t.text for t in out]
    ...
    for p in place:
        if p in data_ind.keys():
            ...
            for x in range(start+1, end, 1):
                if x in table_loc:
                    table_highlights = highlight_TBDs(table[table_loc.index(x)])
                    move_table_after(table_highlights, sd1.paragraphs[-1])
